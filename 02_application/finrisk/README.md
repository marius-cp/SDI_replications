...
The raw Refinitiv data cannot be shared. The intermediate forecast data sets generated by `forecasting.R` are stored in the `data` subfolder. We are currently checking whether these data sets can be made publicly available, subject to data licensing restrictions.