...
#' AP Comment
#' The comparisons with DM here are interesting, but could be interpreted as 
#' unfair. 
#' The DM test considers the sum of both components, while you test each 
#' component separately (which is cool). If one wanted to test whether either of
#' the two components was different (across forecasts, on average), which could 
#' be interpreted as more in line with the DM test, then the two tests would have 
#' to be combined, eg via a Bonferroni correction. That would make the combo test 
#' more consertative and reduce its power. How does the power curve for the combo 
#' test compare with the DM test? (It might still be higher, which would be good 
#' for you guys.
#' 
#' ==> Here I propose to compare the DM test at the 10% level with our two
#' component tests at the 5% level (Bonferroni correction). This allows for a 
#' fair comparison as mentioned above. 
...
rrdsc <- 
  ...
  group_by(setup,tt,type, k) %>% 
  summarise(
    rr = mean(dsc<0.05), #rr = mean(dsc<0.1),
    dscinfo = unique(dscinfo), 
    mcbinfo = unique(mcbinfo),
    j=unique(j), 
    k=unique(k)
